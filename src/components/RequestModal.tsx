...
export const RequestModal: React.FC<RequestModalProps> = ({ isOpen, onClose }) => {
  ...
  const copySummary = () => {
    const summary = `Request: ${formData.subject}\nFrom: ${formData.name} (${formData.email})\n\n${formData.description}`;
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 p-4" dir={dir}>
      <div className="w-full max-w-md bg-white rounded-lg shadow-xl overflow-hidden animate-in fade-in zoom-in duration-200">
        <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center">
          <h2 className="text-xl font-semibold text-slate-900">{t('requestModalTitle')}</h2>
          ...
          {!submitted ? (
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <input 
                  type="text" 
                  className={`w-full rounded-md border ${errors.name ? 'border-red-300' : 'border-slate-300'} px-3 py-2 text-sm focus:ring-primary-500 focus:border-primary-500`}
              ...
              <div>
                ...
                <input 
                  type="email" 
                  className={`w-full rounded-md border ${errors.email ? 'border-red-300' : 'border-slate-300'} px-3 py-2 text-sm focus:ring-primary-500 focus:border-primary-500`}
              ...
              <div>
                ...
                <input 
                  type="text" 
                  className={`w-full rounded-md border ${errors.subject ? 'border-red-300' : 'border-slate-300'} px-3 py-2 text-sm focus:ring-primary-500 focus:border-primary-500`}
              ...
              <div>
                ...
                <textarea 
                  rows={4}
                  className={`w-full rounded-md border ${errors.description ? 'border-red-300' : 'border-slate-300'} px-3 py-2 text-sm focus:ring-primary-500 focus:border-primary-500`}
            ...
            <div className="text-center py-4">
               ...
               <h3 className="text-lg font-semibold text-slate-900 mb-2">{t('successMessage')}</h3>
               <div className="flex justify-center gap-3 mt-6">
                 <button onClick={copySummary} className="text-primary-600 font-medium hover:underline px-4 py-2">
                    {t('btnCopySummary')}
                 </button>
                 <button onClick={handleClose} className="bg-slate-900 text-white px-6 py-2 rounded-md font-medium hover:bg-slate-800">
                    OK
                 </button>
               </div>
            </div>
